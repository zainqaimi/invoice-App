...
import { RiEditLine } from "react-icons/ri";
import { IoMdArrowRoundBack } from "react-icons/io";
import { useProducts, useAddProduct, useUpdateProduct, useDeleteProduct } from "../../api/productApi";
...
const Product: React.FC = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const values = await form.validateFields();
      if (editingKey !== null) {
        updateProduct({ ...values, id: editingKey });
        message.success("Product updated successfully!");
      } else {
        addProduct(values);
        message.success("Product added successfully!");
  ...
  // Filter products only if `products` is a valid array
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: Product) => (
        <Space>
          <span onClick={() => handleEdit(record)}>
            <RiEditLine color="#00a8ec" size={18} />
          </span>
      ...
      <div className="md:flex md:space-y-0 space-y-4 justify-between items-center mb-4">
        ...
        <div className="md:flex md:space-y-0 space-y-4 items-center gap-2">
          ...
          <button
            ...
            className="flex items-center space-x-2 py-2 px-5 rounded-md shadow-md text-white bg-[#00a8ec]"
          >
            <FaPlus /> <span>Add Product</span>
          </button>
        </div>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="weightType" label="Weight Type" rules={[{ required: true }]}>
            <Select placeholder="Select Product Weight">
              <Option value="10 kg">10 kg</Option>
              <Option value="20 kg">20 kg</Option>
            </Select>
          </Form.Item>
          <Form.Item name="packingType" label="Packing Type" rules={[{ required: true }]}>
            <Select placeholder="Select Packing Type">
              <Option value="Box">Box</Option>
              <Option value="Bag">Bag</Option>
            </Select>
          </Form.Item>
          <Form.Item name="retailPrice" label="Retail Price" rules={[{ required: true }]}>
